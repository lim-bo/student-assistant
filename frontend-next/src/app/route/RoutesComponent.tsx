...
import {buildRoute} from "@/app/route/utils/buildYandexMapUrl";
import {TimelineItem} from "@/lib/api";
import Button from "@/components/button";

export default function RoutesComponent({routes}: {routes: TimelineItem[]}) {
  const isRoutesEmpty = routes.length === 0;

  const openMap = () => {
    if (isRoutesEmpty) return;

    const mapsUrl = `https://yandex.ru/maps/?rtext=${buildRoute(routes)}`;
